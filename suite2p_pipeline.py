...
from hb_run_s2p2 import hb_run_s2p2
...
ops = {
        ...
        'diameter': 9, # this is the main parameter for cell detection, 2-dimensional if Y and X are different (e.g. [6 12])
        ...
        # parallel settings
        'num_workers': 10, # 0 to select num_cores, -1 to disable parallelism, N to enforce value
        'num_workers_roi': 10, # 0 to select number of planes, -1 to disable parallelism, N to enforce value
...
#    data_path = [raw_base+animalid+'/'+date+'/'+lbl for lbl in expt_ids]
    ...
    opsEnd=hb_run_s2p2(ops=ops,db=db)
    #if delete_raw:
...
def prepare_db(animalid,date,expt_ids,db,
    ...
    db['fast_disk']= fast_disk # string which specifies where the binary file will be stored (should be an SSD)
